...
class DeltaNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._update_rule = os.getenv("DELTA_UPDATE_RULE", "delta").lower()
        if self._update_rule not in ["delta", "efla"]:
            raise ValueError(f"Unsupported DeltaNet update rule: {self._update_rule}")
        if self._update_rule == "efla":
            warnings.warn(
                "EFLA update rule is enabled. Make sure you know what you are doing.",
            )
    ...
    def forward(
        ...
        **kwargs: Unpack[dict],
        ...
        if self._update_rule == "efla":
            ...
            k2 = (k.float() * k.float()).sum(dim=-1).clamp_min(1e-12)
